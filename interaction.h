...
public:
    Fight(Hero hero, Enemy enemy,int round);    //对战双方信息
    void RoundStart();            //对战每回合开始时重复，执行下列函数
    void ShowCondition();         //展示角色的状态
    //void Order();                 //通过“速度”属性判断双方出手顺序
    void Act(Hero hero);                   //采取某项行动（包括1.普通攻击、2.技能释放、3.道具使用）    
    void Act(Enemy enemy);                   //敌人和自己都会进攻，这里使用重载
    //void SkillChoice();           //技能选择              （目前只支持攻击型技能）
    //void GoodsUsing();            //道具的使用            （目前只支持回血道具）
    //bool Hit();                   //命中计算，通过进攻方的“命中”和受击者的“闪避”属性判断是否命中，要用到随机数    
    int  DamageCalc();             //伤害计算，具体公式为（“总攻击”-“总防御”）* “进攻方技能倍率” *（1-“防守方免伤倍率”）
